function [  ] = M_multiplot_ABA2_updown( M, threshold, n_hold, rlowess_span, overlap_type)
...
if n_hold==0
    ...
    subplot(2,3,6)
    [corrAB, pvalAB] = corrAcorrB_updown(M, rlowess_span, string_type_corr);
%     [corrAB, pvalAB] = corrAcorrB(M, rlowess_span, string_type_corr);
...
%     [corrAB, pvalAB] = corrAcorrB(M, rlowess_span, string_type_corr);
    ...
    subplot(2,4,8)
    [corrAB, pvalAB] = corrAcorrB_updown(M_unhold, rlowess_span, string_type_corr);
%     [corrAB, pvalAB] = corrAcorrB(M_unhold, rlowess_span, string_type_corr);
    str = {strcat('corr = ',num2str(corrAB)),strcat('pval = ',num2str(pvalAB))};
    text(0.1,0.8,str)
